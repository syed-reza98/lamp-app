...
try {
    ...
    // Test query
    $stmt = $pdo->query("SELECT 1 as test, VERSION() as version, NOW() as current_time");
    $result = $stmt->fetch();

    // Check database performance
    $perf_stmt = $pdo->query("SHOW STATUS WHERE Variable_name IN ('Threads_connected', 'Uptime', 'Questions')");
    $performance = [];
...
$health_status['checks']['database']['response_time_ms'] = round((microtime(true) - $db_check_start) * 1000, 2);
