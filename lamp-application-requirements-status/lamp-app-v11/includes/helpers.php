...
{
    return "
    <div class=\"card metric-card hover-lift hover-glow\">
...
function renderStatusCard($icon, $title, $value, $description, $status = 'info')
{
    ...
    return "
    <div class=\"card status-card {$statusClass} hover-lift\">
        <div class=\"icon-wrapper\">{$icon}</div>
        <div class=\"card-content\">
            <h3>{$title}</h3>
...
function getDatabaseConnection($environment = null)
{
    ...
    }    // Validate required environment variables (password can be empty for local development)
    $required_keys = ['rds_hostname', 'rds_port', 'rds_db_name', 'rds_username'];
    ...
    if (!isset($environment['rds_password'])) {
...
function getPerformanceMetrics()
{
    return [
        ...
        'server_load' => function_exists('sys_getloadavg') ? sys_getloadavg() : 'N/A (Windows)',
